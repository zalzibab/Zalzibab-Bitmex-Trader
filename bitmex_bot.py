...
{dict_str(WALLETDICT)}'''
    ...
    if OPENPOSITION != None:
        ...
        ENTRY = usd_str(OPENPOSITION['avgEntryPrice'])
        if SIZE > 0:
            SIDE = 'Buy'
        else:
            SIDE = 'Sell'
...
def cancel_all_orders(account):
    client = load_credentials(account);
    now = datetime.strftime(datetime.utcnow(), '%m-%d-%Y %H:%M:%S')
    my_orders = client.Order.Order_getOrders(filter = json.dumps({'open': 'true', 'symbol': 'XBTUSD'})).result()[0]
    if len(my_orders) != 0:
        msg = f'''{now}
...
def x1_long(account):
    ...
    close_position(account);
    balance = client.User.User_getWalletHistory().result()[0][0]['walletBalance']/100000000
    market_price = client.Instrument.Instrument_get(symbol=symbol).result()[0][0]['askPrice']
    size = int(((market_price + (market_price*0.0015))*balance)*0.995)
    try:
...
def close_position(account):
    ...
    now = datetime.strftime(datetime.utcnow(), '%m-%d-%Y %H:%M:%S')
    cancel_all_orders(account);
    resp = client.Position.Position_get(filter=json.dumps({'isOpen': True, 'symbol': 'XBTUSD'})).result()[0]
    if len(resp) == 0:
        msg = f'''{now}
...
def start(update, context):
    ...
    reply_text = "Welcome to Zalzibab Bitmex Trader\n\n"
...
def main():
    ...
    # Create the Updater and pass it your bot's token.
    pp = PicklePersistence(filename='Zalzibab Bitmex Trader')
